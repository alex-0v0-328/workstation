...
function harness() {
  ...
  const fake = { load: () => structuredClone(state), save: (value: typeof state) => Object.assign(state, value), get: (key: string, fallback: unknown) => cache.has(key) ? structuredClone(cache.get(key)) : fallback, set: (key: string, value: unknown) => cache.set(key, structuredClone(value)) }
